            ...
            for window in d['nodes']:
                    ...
                    if len(window['nodes']) > 0:
                        for n in window['nodes']:
                            ...
                            if 'window_properties' in n:
                                ...
                                class_name = n['app_id']
                            windows.append({'name': n['name'], 'id': n['id'], 'app': class_name})
                    else:
                ...
                if 'floating_nodes' in window:
                    for f in window['floating_nodes']:
                        windows.append({'name': f['name'], 'id': f['id'], 'app': class_name})
        if 'floating_nodes' in d:
            for window in d['floating_nodes']:
                ...
                if 'window_properties' in window:
                    ...
                    class_name = window['app_id']
                windows.append({'name': window['name'], 'id': window['id'], 'app': class_name})


wofi_windows = []
for window in windows:
    if window['name'] != None:
        wofi_windows.append(str(window['app'])+" - "+window['name'].strip().replace('"', "")+" - "\
                +str(window['id']))
...
try:
    output = subprocess.check_output(('wofi', '--dmenu', '-i', '-p', 'Active windows:'), \
            stdin=ps.stdout)
    if output:
        app_id = re.search(r"\- (\d+)$", output.decode('utf-8'))
        if app_id:
            app_id = app_id.group(1)
            subprocess.check_output(('swaymsg', '[con_id="'+str(app_id)+'"]', 'focus'))
except Exception as e:
    pass
